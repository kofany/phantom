...
options::event *entListener::set(const char *_arg, const bool justTest)
{
	...
	{
#ifdef HAVE_WEBAPI
		_event.setError(this, "+listen <ip> <port> <all|users|bots|webapi>", name);
#else
	...
	{
#ifdef HAVE_WEBAPI
		if(strcmp(arg[2], "all") && strcmp(arg[2], "users") && strcmp(arg[2], "bots") && strcmp(arg[2], "webapi"))
		{
		        _event.setError(this, "last argument must be \"all\", \"users\", \"bots\" or \"webapi\"", name);
                        return &_event;
		}
#else
