						...
						do
						{
							...
							if(n > 0)
							{
								if(c->status & STATUS_PARTY)
									parse_owner(c, buf);
#ifdef HAVE_WEBAPI
								else if(c->status & STATUS_WEBAPI)
									parse_webapi(c, buf);
#endif
...
//			net.irc.fd, net.irc.status & STATUS_SSL, net.irc.status & STATUS_SSL_HANDSHAKING));
		...
		if(net.irc.fd && net.irc.status & STATUS_SSL_HANDSHAKING)
		{
			...
			if(net.irc.status & STATUS_CONNECTED)
			{
				...
				// Only use CAP negotiation if SASL is configured
				if(config.sasl_mechanism > 0 && config.sasl_username.getLen() && config.sasl_password.getLen())
				{
					net.irc.send("CAP LS");
