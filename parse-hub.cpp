			...
			case 2:
			{
				if(strlen(arg[3]))
				{
					if(MD5HexValidate(arg[3], net.hub.tmpstr, strlen(net.hub.tmpstr), ((entMD5Hash *) &config.hub.getPass())->getHash(), 16))
					{
						...
						// Use config.handle if not on IRC.
						const char *myname = ME.nick.len() ? (const char *)ME.nick : (const char *)config.handle;
						net.hub.send("%s %s %s %s %s", S_REGISTER, S_VERSION, buf, myname, net.irc.origin);
						return;
					}
				}
				break;
