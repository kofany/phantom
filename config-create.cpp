...
void createInitialConfig(bool expMode)
{
	...
	switch( bottype )
	{
		case 0:
			...
			do
			{
				readUserInput("Listen address and port (e.g. 127.0.0.1 5000)?", config.listenport[n]);
#ifdef HAVE_WEBAPI
				const char *portTypes[] = { "all", "users", "bots", "webapi" };
				int portType = readUserMC("Listen type?", portTypes, 4, 0);
#else
				const char *portTypes[] = { "all", "users", "bots" };
				int portType = readUserMC("Listen type?", portTypes, 3, 0);
#endif
				if(portType > 0)
				{
					char tmp[256];
					snprintf(tmp, sizeof(tmp), "%s %s", config.listenport[n].getValue(), portTypes[portType]);
					config.listenport[n] = tmp;
				}
				n++;
			} while(n < MAX_LISTENPORTS && readUserYesNo("Add another listen port?", false));
...
/*	if( readUserYesNo( "Do you want to set up the optional stuff?", false ) )
	{
		...
		readUserInput( "Which ctcp version do you want? 0 = none, 1 = Phantom, 2 = irssi, 3 = epic, 4 = lice, 5 = bitchx, 6 = dzony loker, 7 = luzik, 8 = mirc 6.14", config.ctcptype );
...
void expertMode()
{
  ...
  printMessage("set nick phantom - sets variable \"nick\" to value \"phantom\"");
