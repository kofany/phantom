...
//                listening=true;
	...
	if(listening_ssl)
	{
		...
		printf("[*] Loading private key from `%s/%s'\n", cwd, tmp);
		if(!SSL_CTX_use_PrivateKey_file(inet::server_ctx, tmp, SSL_FILETYPE_PEM))
		{
...
#ifdef HAVE_WEBAPI
			else if(!strcmp(config.listenport[i].getHandle(), "webapi"))
				e->access=LISTEN_WEBAPI;
#endif
