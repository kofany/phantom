...
void client::sendStatus(const char *name)
{
	...
	net.sendUser(name, "Hi. I'm %s and I'm running Phantom %s", (const char *) config.handle, S_VERSION);
...
#ifdef HAVE_ADNS
	// LEAF ONLY: Clear DNS cache once when not resolving, so reconnects use fresh DNS.
	// Only clear if we're not currently in the middle of a resolve
	if(config.bottype == BOT_LEAF && !config.hub.getHost().resolve_pending)
	{
		// Helper to strip ssl: prefix for cache lookup
		auto stripPrefix = [](const char *host) -> const char* {
			if(!host) return host;
			if(!strncmp(host, "ssl:", 4)) return host + 4;
			if(!strncmp(host, "ipv4:", 5)) return host + 5;
			if(!strncmp(host, "ipv6:", 5)) return host + 5;
			return host;
		};

		// Only clear cache, don't touch ip4/ip6/resolve_pending - let updateDnsEntry handle those
		if(config.hub.getHost().connectionString.len())
		{
			const char *hubHost = config.hub.getHost().connectionString;
			resolver->removeCache(stripPrefix(hubHost));
		}
	}
#endif
