...
#ifdef HAVE_WEBAPI
				else if(status & STATUS_WEBAPI)
				{
					net.send(HAS_N, "[WebAPI] %s disconnected (%s)", handle->name, reason);
					webapi_broadcast_user_quit(handle->name);
				}
#endif
...
#ifdef HAVE_WEBAPI
					webapi_broadcast_bot_quit(handle->name, reason);
#endif
				}
			}
...
int inetconn::enableLameCrypt()
{
	...
	if(!fstat(fd, &info))
	{
		...
		if(!info.st_size)
		{
			strcpy(h.id, "phantom");
			h.version = version;
			::write(fd, &h, sizeof(phantomHeader));
		}
		else
		{
			::read(fd, &h, sizeof(phantomHeader));
			if(!strcmp(h.id, "phantom"))
			{
