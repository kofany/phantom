...
bool ul::isBotByFD(int fd)
{
	...
	c->fd=0;  // Prevent destructor from closing the socket
	delete c;
	return ret;
}
...
{
	HANDLE *p = first;
	int globalNeed = HAS_ALL, chanNeed = HAS_ALL;
	int globalGot = 0, chanGot = 0;
	...
	while(p)
	{
		...
		else
		{
			if(wildFindHost(p, mask) != -1)
			{
				// Collect global flags separately
				int gFlags = globalNeed & p->flags[MAX_CHANNELS];
				if(gFlags)
					globalNeed -= gFlags;

				// Collect channel flags separately
				int cFlags = chanNeed & p->flags[ch->channum];
				if(cFlags)
					chanNeed -= cFlags;
			}
		}
	...
	globalGot = HAS_ALL - globalNeed;
	chanGot = HAS_ALL - chanNeed;

	// Combine flags
	int got = globalGot | chanGot;

	// Channel negative flags (+d, +q, +k) override global positive flags
	// EXCEPT for users with +x or +s (they are immune)
	if(!(globalGot & (HAS_X | HAS_S)))
	{
		// If channel has +d, remove op-related flags
		if(chanGot & HAS_D)
		{
			got &= ~(HAS_O | HAS_M | HAS_N | HAS_F);
			got |= HAS_D;
		}
		// If channel has +q, remove voice flag
		if(chanGot & HAS_Q)
		{
			got &= ~HAS_V;
			got |= HAS_Q;
		}
		// If channel has +k, user gets kicked
		if(chanGot & HAS_K)
		{
			got = HAS_K;
		}
	...
	{
		// +x and +s users ignore channel negative flags
		got &= ~(HAS_D | HAS_Q | HAS_K);
	}

	// Standard flag cleanup
	if(got & HAS_O) got &= ~(HAS_V | HAS_Q);
...
//	if(!i) return 4; /* its checked in addIdiot() method. Should we check id second time? */
	...
	switch(i)
	{
		...
		case 1:
			...
			break;
		case 6:
			// Op lockdown - disable autoop for users below +n
			userlist.chanlist[channum].chset->OP_LOCKDOWN = 1;
			net.send(HAS_B, "%s %s %s %d", S_CHSET, (const char *) userlist.chanlist[channum].name, "op-lockdown", 1);
			net.send(HAS_N, "\0034Op lockdown enabled on %s - autoop disabled for users below +n\003", (const char *) userlist.chanlist[channum].name);
			break;
	}
        return 0;
}
