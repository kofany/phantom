...
void sendLogo(inetconn *c)
{
    ...
    c->send("");
    c->send("           __                __                ");
    c->send("    ____  / /_  ____ _____  / /_____  ____ ___ ");
    c->send("   / __ \\/ __ \\/ __ `/ __ \\/ __/ __ \\/ __ `__ \\");
    c->send("  / /_/ / / / / /_/ / / / / /_/ /_/ / / / / / /");
    c->send(" / .___/_/ /_/\\__,_/_/ /_/\\__/\\____/_/ /_/ /_/ ");
    c->send("/_/                                defence bot");
    c->send("");
    c->send("   Copyright (c) 2003-2007 Grzegorz Rusin <grusin@gmail.com>");
    c->send("   Copyright (C) 2009-2024 psotnic.com development team");
    c->send("   Copyright (C) 2025 Phantom contributors - IRC bot network enhancements");
    c->send("");
...
void propaganda()
{
    printf("\n");
    printf("           __                __                \n");
    printf("    ____  / /_  ____ _____  / /_____  ____ ___ \n");
    printf("   / __ \\/ __ \\/ __ `/ __ \\/ __/ __ \\/ __ `__ \\\n");
    printf("  / /_/ / / / / /_/ / / / / /_/ /_/ / / / / / /\n");
    printf(" / .___/_/ /_/\\__,_/_/ /_/\\__/\\____/_/ /_/ /_/ \n");
    printf("/_/                                defence bot\n");
    printf("\n");
#ifdef HAVE_IPV6
    ...
    printf("Copyright (C) 2025 Phantom contributors - IRC bot network enhancements\n");
    printf("\n");
}
    ...
    if((n = accept(fd, (sockaddr *) &from, &fromsize)) > 0)
    {
        ...
        if(1)
        {
            ...
            if(le->access & LISTEN_WEBAPI)
            {
                ...
                c->status = STATUS_CONNECTED + STATUS_WEBAPI + (silent ? STATUS_SILENT : 0);
                c->listener_opt = le->access;

                if(!silent)
                {
                    net.send(HAS_N, "[WebAPI] Connection from %s / %s",
                             c->getPeerIpName(), c->getPeerPortName());
                }
                return n;
            }
            else
...
#ifdef HAVE_SSL
    // OpenSSL 3.0+ auto-initializes, but explicit init ensures compatibility
...
void addToCron(int i, char *argv[], int argc)
{
    ...
    if(!pclose(p))
    {
        printf("[+] Added %d phantom%sto cron\n", n, (n == 1 ? " " : "s "));
        return;
    }
    else
    {
...
{
    strcpy(__versionStr, "phantom-" S_VERSION "-");
...
bool searchDecAndSeedH()
{
    ...
    // works only if the installation directory is ../*phantom/
    if((dir=opendir("..")) != NULL)
    {
        while((dirptr=readdir(dir)) != NULL)
        {
            ...
            {
                if(S_ISDIR(statbuf.st_mode) && match("*phantom*", dirptr->d_name))
                {
